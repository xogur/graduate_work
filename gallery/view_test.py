...
#from .models import Image, show_Image

""" from .forms import MultipleImageFormz
...
""" def show_images(request):
    images = show_Image.objects.order_by('-uploaded_at')[:2]  # 최신 이미지 두 개만 가져오기
    context = {'images': images}
    return render(request, 'gallery/show_images.html', context) """

""" def show_images(request):
    images = Image.objects.order_by('-uploaded_at')[:1]  # 최신 이미지 두 개만 가져오기
    context = {'images': images}
    return render(request, 'gallery/show_images.html', context)
 """
